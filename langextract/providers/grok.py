...
"""Grok (XAI) provider for LangExtract."""
...
@registry.register(
    r'^grok-',  # grok-beta, grok-v2, etc.
    priority=10,
)
@dataclasses.dataclass(init=False)
class GrokLanguageModel(inference.BaseLanguageModel):
  """Language model inference using XAI's Grok API with structured output."""
  ...
  def __init__(
    ...
    """Initialize the Grok (XAI) language model.

    Args:
      model_id: The Grok model ID to use (e.g., 'grok-beta').
      api_key: API key for XAI service.
      base_url: Base URL for XAI service (default is XAI's API endpoint).
    ...
    """
    # Lazy import: OpenAI package required (XAI uses OpenAI-compatible API)
    try:
      # pylint: disable=import-outside-toplevel
      import openai
    except ImportError as e:
      raise exceptions.InferenceConfigError(
          'Grok provider requires openai package. '
    ...
    if not self.api_key:
      raise exceptions.InferenceConfigError('API key not provided for Grok (XAI).')
  ...
  def _process_single_prompt(
    ...
    try:
      ...
      raise exceptions.InferenceRuntimeError(
          f'Grok (XAI) API error: {str(e)}', original=e
      ) from e

  def infer(
      self, batch_prompts: Sequence[str], **kwargs
  ) -> Iterator[Sequence[inference.ScoredOutput]]:
    """Runs inference on a list of prompts via Grok's API.
