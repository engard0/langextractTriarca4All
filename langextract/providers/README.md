# LangExtractTriarca4All Provider System

This directory contains the provider system for LangExtractTriarca4All, which is a derivative work based on LangExtract by Google LLC, extended with additional Large Language Model (LLM) backends.
...
- Full class name: `"GeminiLanguageModel"`, `"OpenAILanguageModel"`, `"OllamaLanguageModel"`, `"AnthropicLanguageModel"`, `"MistralLanguageModel"`, `"GrokLanguageModel"`
- Partial match: `"gemini"`, `"openai"`, `"ollama"`, `"anthropic"`, `"mistral"`, `"grok"` (case-insensitive)
...
- **Anthropic** (`anthropic.py`): Anthropic's Claude models
  - Code included in package
  - Requires: `pip install langextract[anthropic]` to install Anthropic SDK
- **Mistral** (`mistral.py`): Mistral's models
  - Code included in package
  - Requires: `pip install langextract[mistral]` to install Mistral SDK
- **Grok** (`grok.py`): XAI's Grok models
  - Code included in package
  - Requires: `pip install langextract[openai]` to install OpenAI SDK (Grok uses OpenAI-compatible API)
...
,  # Explicit: model_id="GeminiLanguageModel"
...
```
**Solution**: Install optional dependencies: `pip install langextract[openai]`
,  # Explicit: model_id="AnthropicLanguageModel"
    r'^claude-'                  # Prefix: model_id="claude-3-5-sonnet-latest"
)
...
```
**Solution**: Install optional dependencies: `pip install langextract[openai]`
,   # Explicit: model_id="MistralLanguageModel"
    r'^mistral-'                # Prefix: model_id="mistral-large-latest"
)
class MistralLanguageModel(lx.inference.BaseLanguageModel):
    def __init__(self, model_id: str, api_key: str = None, **kwargs):
...
```
**Solution**: Install optional dependencies: `pip install langextract[openai]`
,      # Explicit: model_id="GrokLanguageModel"
    r'^grok-'                   # Prefix: model_id="grok-beta"
)
class GrokLanguageModel(lx.inference.BaseLanguageModel):
    def __init__(self, model_id: str, api_key: str = None, **kwargs):
